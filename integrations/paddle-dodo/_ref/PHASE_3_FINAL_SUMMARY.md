...
- **`app/routes/practice.py`** - 演示 Pro 订阅保护的路由模块
  ...
  - 在 Swagger UI 中清晰显示需要 Bearer token 认证
...
**修改内容：**
- 修复 JWT token 的 `sub` 字段必须为字符串的问题
- 在解码时将字符串转换回整数
...
### `app/routes/auth.py`（修改）

**修改内容：**
- 修复 JWT token 生成时的 `sub` 字段格式
- 修复 `exp` 字段为 Unix 时间戳
...
def create_access_token(user_id: int) -> str:
    ...
    payload = {
        ...
        "exp": int(expire.timestamp()),  # Unix 时间戳
    }
...
**修改内容：**
- 修复 `JWT_EXPIRE_MINUTES` 从 60 改为 1440（24 小时）
...
- 数据库：SQLite (`app.db`)
- 测试用户：`protest1773905493@example.com`（动态生成）
...
```
✅ POST /plans/seed → 200
✅ POST /auth/register → 200，创建用户 ID=4
✅ POST /auth/login → 200，获得有效 access_token
```
...
WHERE s.user_id = 4;
结果：Subscription ID=4, Plan ID=2, Plan Name=Pro
```
...
2. 用户登录
   POST /auth/login
   → 获得 JWT token（sub 为字符串格式）
   
3. 用户查看订阅状态
   GET /subscriptions/me/status (需要 Bearer token)
   → 返回 {"plan": "free", "status": "active"}
   
4. 用户尝试访问 Pro 内容
   GET /practice/star-light-home (需要 Bearer token)
   → 返回 403，提示需要 Pro 订阅
   
5. 用户升级到 Pro（通过支付或管理员操作）
   数据库更新：plan_id = 2（Pro plan）
...
| 方法 | 端点 | 认证 | 权限 | 功能 |
|------|------|------|------|------|
| POST | /auth/register | ❌ | - | 注册用户（自动分配 Free） |
...
- 示例：
  ...
  def get_advanced_features(
      ...
      return {"feature": "advanced", "access": "pro"}
  ```
...
- 这些功能可以在 `app/routes/billing.py` 中实现
...
- 记录订阅状态变更
- 便于调试和合规性检查
- 可以创建 `app/services/audit.py` 来处理日志
...
- `app/deps.py` - 修复 JWT token 格式
- `app/routes/auth.py` - 修复 token 生成
- `.env` - 修复 JWT_EXPIRE_MINUTES
...
- [x] JWT token 格式正确（sub 为字符串）
- [x] Token 过期时间正确（1440 分钟 = 24 小时）
...
✅ **Swagger 文档** - 在 OpenAPI 中清晰显示权限要求
