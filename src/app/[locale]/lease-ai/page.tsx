...
export default function LeaseAIPage() {
  ...
  const handleStartTrial = async () => {
    ...
    try {
      ...
      const response = await fetch(`${authTrialUrl}/api/me/products/start-trial`, {
        ...
        body: JSON.stringify({ productKey: "lease-ai", userEmail: session.user.email }),
      });
      ...
      if (!response.ok) {
        ...
        if (errorCode === "TRIAL_ALREADY_USED") {
          setError(isZh ? "你已经使用过试用，请直接购买后继续使用。" : "You have already used your trial. Please purchase to continue.");
          handleBuy();
          return;
        }
        setError(isZh ? "开始试用失败，请稍后重试。" : "Failed to start trial. Please try again.");
        return;
      }
      window.location.href = "https://app.tutorbox.cc/us-lease";
    } catch (err) {
      console.error("Failed to start trial:", err);
      setError(isZh ? "发生错误，请稍后重试。" : "An error occurred. Please try again.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleBuy = async () => {
    ...
    try {
      ...
      if (!priceId) {
        setError(isZh ? "支付暂时不可用，请稍后再试。" : "Purchase is not available at this time.");
        return;
      }
      await openCheckout({ priceId, userId: session.user.id });
    } catch (err) {
      console.error("Failed to open checkout:", err);
      setError(isZh ? "打开支付窗口失败，请稍后再试。" : "Failed to open checkout. Please try again.");
    }
  };
      ...
      <div className="mx-auto max-w-4xl px-4 py-16 sm:py-24">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-base text-muted-foreground">
            {isZh ? "30 秒读懂合同陷阱，保护你的租房权益" : "Understand lease traps in 30 seconds"}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-12 text-center">
          <div className="bg-card border rounded-lg p-6">
            ...
            <h4 className="font-semibold mb-1">{isZh ? "30 秒极速分析" : "30-Second Analysis"}</h4>
          ...
          <div className="bg-card border rounded-lg p-6">
            ...
            <h4 className="font-semibold mb-1">{isZh ? "陷阱条款识别" : "Trap Detection"}</h4>
            <p className="text-sm text-muted-foreground">
              {isZh ? "识别隐藏费用、不公平条款和违约风险" : "Spot hidden fees, unfair clauses & penalties"}
            </p>
          </div>
          <div className="bg-card border rounded-lg p-6">
            <div className="text-2xl mb-2">💬</div>
            <h4 className="font-semibold mb-1">{isZh ? "中英双语解释" : "Bilingual Support"}</h4>
            <p className="text-sm text-muted-foreground">
              {isZh ? "用中文读懂英文合同，无需法律背景" : "Understand English leases in plain Chinese"}
            </p>
          </div>
        </div>
          ...
          <div className="bg-card border rounded-lg p-8 shadow-sm max-w-md mx-auto w-full">
            ...
            <p className="text-xs text-muted-foreground mt-6">
              {!session?.user
                ...
                : (isZh ? "一次性付费，无自动续费" : "One-time purchase. No subscription required.")}
            </p>
          </div>
        </div>
        ...
        <div className="mt-16 mb-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 text-center">
            <div>
              <div className="w-10 h-10 rounded-full bg-primary text-primary-foreground flex items-center justify-center font-bold mx-auto mb-3">1</div>
              <h4 className="font-semibold mb-1">{isZh ? "上传合同" : "Upload Lease"}</h4>
              <p className="text-sm text-muted-foreground">
                {isZh ? "支持 PDF 或图片格式" : "PDF or image format supported"}
              </p>
            </div>
            <div>
              <div className="w-10 h-10 rounded-full bg-primary text-primary-foreground flex items-center justify-center font-bold mx-auto mb-3">2</div>
              <h4 className="font-semibold mb-1">{isZh ? "AI 自动分析" : "AI Analyzes"}</h4>
              <p className="text-sm text-muted-foreground">
                {isZh ? "30 秒内完成全文扫描" : "Full scan completed in 30 seconds"}
              </p>
            </div>
            <div>
              <div className="w-10 h-10 rounded-full bg-primary text-primary-foreground flex items-center justify-center font-bold mx-auto mb-3">3</div>
              <h4 className="font-semibold mb-1">{isZh ? "获得分析报告" : "Get Insights"}</h4>
              <p className="text-sm text-muted-foreground">
